...
class JsonBodyMatcherSpec extends Specification {
  ...
  def 'matching json bodies - with ignore-order - return mismatches on nested lists when overriding equality'() {
    ...
    then:
    mismatches.size() == 1 + 2 * (1 + 3) + (1 + 3 * 3)
    mismatches*.path == ['$'] + // top-level unordered mismatch
        ['$.0', '$.0.0', '$.0.1', '$.0.2'] + // [6,4,5] != [1,2,3]
        ['$.0', '$.0.0', '$.0.1', '$.0.2'] + // [2,3,1] != [1,2,3]
        ['$.1'] + // unordered mismatch of [2,3,1] with [4,5,6]
        ['$.1.0', '$.1.1', '$.1.2'] + // 2 doesn't match 4, 5, or 6
        ['$.1.0', '$.1.1', '$.1.2'] + // 3 doesn't match 4, 5, or 6
        ['$.1.0', '$.1.1', '$.1.2'] // 1 doesn't match 4, 5, or 6
  }
